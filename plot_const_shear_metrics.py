...
NTRUESETS = 50           # Don't necessarily need to have NIMS input shears. But easiest if NTRUE an
...
for j in range(NMONTE):
    ...
    for i in range(NBINS):
        g1sub, g2sub = g3utils.make_submission_const_shear(CFID, CFID, mvals[i], mvals[i],
                                                           g1true, g2true, ngals_per_im=NGALS_PER_IM,
                                                           noise_sigma=NOISE_SIGMA)
        Q08_vs_m[i] = g3utils.metricQ08_const_shear(g1sub, g2sub, g1true, g2true, ntruesets=NTRUESETS)
...
for j in range(NMONTE):
    ...
    for i in range(NBINS):
        g1sub, g2sub = g3utils.make_submission_const_shear(cvals[i], cvals[i], MFID, MFID,
                                                           g1true, g2true, ngals_per_im=NGALS_PER_IM,
                                                           noise_sigma=NOISE_SIGMA)
        Q08_vs_c[i] = g3utils.metricQ08_const_shear(g1sub, g2sub, g1true, g2true, ntruesets=NTRUESETS)
